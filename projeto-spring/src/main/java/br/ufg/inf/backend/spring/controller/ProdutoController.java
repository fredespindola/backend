...
public class ProdutoController {
	...
	public String editarProduto(@RequestParam("id") Long id, @RequestParam("nome") String nome, @RequestParam("preco") double preco,
			RedirectAttributes redirectAttributes) {
		Produto produto = produtoService.obterProdutoById(id);
		produto.setNome(nome);
		produto.setPreco(preco);
		produtoService.salvarProduto(produto);
		redirectAttributes.addAttribute("sucesso", "Produto editado com sucesso!");
		return "redirect:/produtos";
	}
	
	@PostMapping("/produtos/excluir")
	public String excluirProduto(@RequestParam("id") Long id, RedirectAttributes redirectAttributes) {
